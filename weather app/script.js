...
function getWeatherData(city, unit, hourlyorWeek) {
  fetch(
    `https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services/timeline/${city}?unitGroup=metric&key=N52NAZDRKBL3Z7XUTZVYQ6JSY&contentType=json`,
    {
      method: "GET",
      headers: {},
    }
...
function updateForecast(data, unit, type) {
  ...
  for (let i = 0; i < numCards; i++) {
    ...
    `;
    // Add event listeners for card interactivity
    card.addEventListener("mouseover", () => {
      card.style.backgroundColor = "#c0c0c0"; // Change background color on mouseover
    });
    card.addEventListener("mouseout", () => {
      card.style.backgroundColor = "#fff"; // Revert back to default background color on mouseout
    });
...
function celciusToFahrenheit(temp) {
  return Math.round(((temp * 9) / 5 + 32).toFixed(1));
}
